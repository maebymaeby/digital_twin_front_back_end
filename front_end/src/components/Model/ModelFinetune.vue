<template>
  <el-row class="modelfinetune_framework_finetuneing" v-if="isFinetuneing">
    <loading-component class="loading_layout" :loadingEvent="this.loadingEvent"></loading-component>
  </el-row>
  <el-row class="modelfinetune_framework" v-else>
    <!-- 筛选框部分 -->
    <el-col class="finetune_filter_layout" :span="24">
      ...
      <el-col :span="8" :offset="11" class="filter_input_content">
        <el-select v-model="model_type_selected" class="filter_select" placeholder="全部模型" @change="finetuneSelectChange()" clearable>
          <el-option v-for="item in model_type_list" :key="item.value" :label="item.label" :value="item.value"></el-option>
        </el-select>
        <el-input v-model="model_name_inputted" class="filter_input" placeholder="请输入模型名称" @change="finetuneInputChange()" clearable>
          <i slot="prefix" class="el-input__icon el-icon-search"></i>
        </el-input>
      </el-col>
    </el-col>
    <!-- 表格部分 -->
    <el-col class="finetune_table_layout" :span="24">
      <el-table class="finetune_table" :data="tableData" :header-cell-style="tabelHeaderCellStyle" max-height="390" border>
        <template slot="empty">
        ...
        <el-table-column prop="model_operation" min-width="120" fixed="right" align="center">
          <template slot="header">
            <p class="table_title">增量学习</p>
          </template>
          <template slot-scope="scope">
            <div class="select_operation">
              <el-button class="select_button" v-if="scope.row.isSelected" type="success" @click="selectModel(scope.row)" icon="el-icon-edit" circle></el-button>
              <el-button class="select_button" v-else type="info" @click="selectModel(scope.row)" icon="el-icon-edit" circle></el-button>
            </div>
          </template>
        </el-table-column>
      </el-table>
        ...
        if (row.isSelected == true) {
          ...
          for (let i = 0; i < this.tableData.length; i++) {
            ...
            if (row.model_id == this.tableData[i].model_id) {
              ...
              if (row.model_type.indexOf('GA') != -1) {
                this.model_type_clicked_db = "ga";
              }
              else {
                this.model_type_clicked_db = "pso";
              }
            }
            row.isSelected = true;
          }
      ...
      excelUploadSuccess(excelData) {
        if (this.modelIndex == -1) {
          ...
          if (this.model_type_clicked.indexOf('GA') != -1 || this.model_type_clicked.indexOf('PSO') != -1) {
              ...
              return new Promise((resolve, reject) => {
                STORE
                  ...
                  .then((res) => {
                    ...
                    modelStr = (modelStr.replace(':', '_')).replace('-', '_') + '_' + this.model_type_clicked_db
                    let trainLoss = 0
                    for (let i = 0; i < res.data.loss.length; i++) {
                      trainLoss += res.data.loss[i]
                    }
                    let model_info = {
                      "model_id": modelStr,
                      "model_type": this.model_type_clicked_db,
                      "create_time": timeStr,
                      "train_length": this.requestData.input.length,
                      "train_loss": trainLoss
                    }
                    let resData = {
                      "model_info": model_info,
                      "model_result": res.data,
                    }
                    this.$router.push({
            ...
            return new Promise((resolve, reject) => {
              STORE
                .dispatch("Model/modelFinetune", this.requestData)
                .then((res) => {
                  this.$router.push({
                ...
                .catch((error) => {
                  this.isFinetuneing = false;
                  this.tableData[this.selectedRowIndex].isSelected = false;
...
<style>
  .modelfinetune_framework_finetuneing {
    height: 600px;
    margin: 16px;
    padding: 24px;
