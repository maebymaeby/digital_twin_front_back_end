<template>
  <el-row class="mechanismmodeling_framework" v-if="isTraining">
    <loading-component class="loading_layout" :loadingEvent="this.loadingEvent"></loading-component>
  </el-row>
  <el-row class="mechanismmodeling_framework" v-else>
...
<script>
  ...
  export default {
    ...
    name: "MechanismModeling",
    data() {
      return {
        // 模型卡片信息
        modelItems: [
        {
          content: '粒子群优化是一种模拟简化社会模型的优化算法',
          index: 0,
          isSelected: false,
          src: require('@/assets/PSO.png'),
          title: 'PSO（粒子群算法）',
        },
        {
          content: '遗传算法是一种模拟生物进化论中自然选择和遗传学机理的优化算法',
          index: 1,
          isSelected: false,
    ...
    methods: {
      ...
      excelUploadSuccess(excelData) {
        if (this.modelIndex == -1) {
          ...
          return new Promise((resolve, reject) => {
            STORE
              .dispatch("Model/mechanismModelTrain", this.requestData)
              .then((res) => {
                ...
                let model_info = {
                  "model_id": this.requestData.id,
                  "model_type": this.requestData.model_type,
                  "create_time": timeStr,
      ...
      generateRequestData(model_type, processedData) {
        ...
        modelStr = (modelStr.replace(':', '_')).replace('-', '_') + '_' + model_type
